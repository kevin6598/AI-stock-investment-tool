...
export default function PredictionCard({ prediction }: Props) {
  ...
  return (
    <div className="bg-card dark:bg-card light:bg-card-light rounded-xl p-6 border border-gray-700 dark:border-gray-700">
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-2xl font-bold">{p.ticker}</h2>
          <span className="text-sm text-gray-400">{p.horizon} forecast</span>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div>
          <div className="text-xs text-gray-400 mb-1">P(Up)</div>
        ...
        <div className="flex items-end justify-between h-16 gap-1">
          {[
            ...
            <div key={q.label} className="flex-1 text-center">
              ...
              <div className="text-[9px] text-gray-500">{q.label}</div>
            </div>
          ))}
        </div>
      </div>
